...
# вопрос: почему символ \ в списке становится \\, но определяется как \ ?

# не выдержал - залез в google:
# if name_file.startswith(('@', '№', '$', '%', '^', '&', '*', '(', ')')):
# я пробовал так решить, Но 
# 1 - это надо каждый символ прописывать в ковычки, считаю это не правильно
# 2 - проставить ковычки через код в строке на каждый символ я не смог
# 3 - почему двойные скобки?

# на первый взгляд простая задача, а оказалось столько проблем ))))))
# либо я не разобрался в чем-то