...
# Первым делом проверяем, зарегистрирован ли пользователь. Если нет, 
    # направляем его на /start.

# Подробнее рассмотрим следующие строки:
    tasks: List[Task] = user.tasks.order_by(-Task.due_date, -Task.task_id).limit(10)
    
    result.extend(map(str, reversed(tasks)))
    # Помните о backref? Здесь мы как раз используем его, чтобы получить список задач 
    # пользователя с помощью user.tasks.
    # Далее с помощью order_by упорядочиваем задачи сначала по убыванию даты, 
    # а затем по убыванию ID.
    # С помощью limit получаем первые десять результатов. Таким образом, мы получим 
    # последние десять задач. Однако они будут идти в обратном хронологическом 
