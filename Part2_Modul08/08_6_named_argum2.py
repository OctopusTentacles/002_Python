...
# При работе со значениями по умолчанию и изменяемыми типами данных 
# нужно знать и остерегаться ещё одной интересной штуки.
...
# В основной программе вызовите функции три раза только 
# с одним аргументом (числом), например так:
...
def add_num2(num, lst=None):
    lst = lst or []
    # хитрая конструкция, которая позволит упростить ввод:
    # if not lst:
    #    lst = []
    # Первый вариант будет выбран, если lst не равен None, иначе будет создан и записан пустой список
    lst.append(num)
    print(lst)
