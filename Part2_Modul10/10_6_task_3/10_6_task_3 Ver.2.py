            ...
            if list_line[0].isalpha():
                if ('@' in list_line[1]) and ('.' in list_line[1]):                       
                    if 9 < int(list_line[2]) < 100:
                        ...
                        raise Exception('Поле «Возраст» НЕ''является числом от 10 до 99')
                else:
                    raise Exception('Поле «Имейл» НЕ содержит @ и . (точку)')       
            else:
                raise Exception('Поле «Имя» содержит НЕ только буквы')
